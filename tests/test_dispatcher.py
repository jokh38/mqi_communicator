...
def test_run_case_level_ptn_checker_analysis_records_plural_delivery_report_paths(mock_settings):
    dispatcher = importlib.import_module("src.core.dispatcher")
    logger = MagicMock()
    case_repo = MagicMock()
    case_repo.get_deliveries_for_case.return_value = [
    ...
    integration.run_analysis.return_value = dispatcher.PtnCheckerResult(
        ...
        output_dir=Path("tmp") / "Daily" / "case-1",
        analysis_data=beam_metrics,
        report_path=Path("tmp") / "Daily" / "case-1" / "beam_1_summary.pdf",
        report_paths=[
    ...
    assert result.success is True
    assert case_repo.record_delivery_analysis_result.call_args.kwargs["report_path"] == (
        Path("tmp") / "Daily" / "case-1" / "beam_1_summary.pdf"
    )
    assert case_repo.record_delivery_analysis_result.call_args.kwargs["report_paths"] == [
        Path("tmp") / "Daily" / "case-1" / "beam_1_summary.pdf",
        Path("tmp") / "Daily" / "case-1" / "beam_1_detail.pdf",
    ]
